...
type ProductModalProps = {
  activeProductId: ProductIds | null;
  onClose: () => void;
};
type ImageTransition = 'left' | 'right' | null;

export default function ProductModal({ activeProductId, onClose }: ProductModalProps) {
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const [imageTransition, setImageTransition] = useState<ImageTransition>(null);
  ...
  const imagesList = activeProduct ? activeProduct.images : [];
  const [nextImageIndex, setNextImageIndex] = useState<number | null>(null);

  const handleImageChange = (nextIndex: number, direction: ImageTransition) => {
    if (imagesList.length <= 1 || nextImageIndex !== null) return;
  ...
  const handlePrevImage = () => {
    ...
    const nextIndex = (currentImageIndex - 1 + imagesList.length) % imagesList.length;
  ...
  return (
    <div className={`modal-overlay ${activeProduct ? 'active' : ''}`} id="product-modal" onClick={handleOverlayClick} data-lenis-prevent>
      {activeProduct && (
              ...
              <div className="modal-img-container">
                ...
                <div className="modal-img-stack">
                  <img
                    ...
                    className={`modal-product-img ${imageTransition ? `slide-out-${imageTransition}` : ''}`}
                  />
                  {nextImageIndex !== null && (
                    <img
                      key={`next-${nextImageIndex}`}
                      src={imagesList[nextImageIndex]}
                      alt={`Voltigen ${activeProduct.title}`}
                      className={`modal-product-img slide-in-${imageTransition}`}
                    />
                  )}
                </div>
